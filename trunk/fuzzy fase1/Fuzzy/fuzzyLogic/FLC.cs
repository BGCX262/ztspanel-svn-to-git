...
namespace Fuzzy.fuzzyLogic
{
    public class FLC
    {
        ...
        {
            if (configuration.Implication == ImpMethod.Min)
            {
                if (value1 < value2) { return value1; }
                else { return value2; }
            }
            else if (configuration.Implication == ImpMethod.Prod)
            {
        ...
        public double DeFuzzification(List<FuzzySet> Sets, LingVariable variable)
        {
            ...
            for (int i = 0; i < Sets.Count; i++)
            {
                ...
                {
                    if (configuration.DefuzzificationType == DefuzzifcationType.ModifiedHeight)
                    {
                        value = MaxHeightDeFuzzifcation(Sets[i].Set, variable);
                    }
                    else if (configuration.DefuzzificationType == DefuzzifcationType.Centroid)
                    {
